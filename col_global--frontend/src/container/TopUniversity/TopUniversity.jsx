...
const Card = ({ imgSrc, title, description, link }) => (
     <div className="max-w-sm h-full border rounded-2xl mx-1 md:mx-2 flex flex-col">
          ...
          <div className="md:p-5 p-1 flex flex-col justify-between flex-grow">
               <div>
                    <Link to={link}>
                         <h5 className="mb-2 text-base md:text-lg font-bold tracking-tight text-gray-900 dark:text-white truncate">{title}</h5>
                    </Link>
                    <p className="md:mb-3 font-normal text-gray-700 dark:text-gray-400 text-xs md:text-base truncate">{description}</p>
               </div>
